...
class PPOBasic(BasicAgent, AgentWithModel):
    def __init__(self, mode="train", model=None) -> None:
        ...
        self.actions_units = [
            self.unit_get_resources,
            self.unit_build_city,
            self.unit_continue_mission,
        ...
        self._fuel_collected_last = None
        self._city_tiles_last = None
        self._units_last = None

        self.logging = False

    def log(self, message: str):
        if self.logging:
            print(f'PPOBasic: {message}')

    def unit_get_resources(self, game, team, unit_id, unit, target_type_restriction=None, **kwarg) -> act.Action:
        """Note: MyUnits game state already updated at beginning of observation"""
        unit = self.units[unit_id]
        return unit.get_resources(resource_type=C.RESOURCE_TYPES.WOOD)

    def unit_build_city(self, game, team, unit_id, unit, target_type_restriction=None, **kwarg) -> act.Action:
        """Note: MyUnits game state already updated at beginning of observation"""
        unit = self.units[unit_id]
        return unit.build_city()

    def unit_continue_mission(self, game, team, unit_id, unit, target_type_restriction=None, **kwarg) -> act.Action:
        """Note: MyUnits game state already updated at beginning of observation"""
        unit = self.units[unit_id]
        return unit.continue_mission()

    def city_build_worker(self, game, team, city_id, citytile: CityTile, x, y, **kwargs) -> act.Action:
        city = self.cities[city_id]
        # self.log('building worker')
        return act.SpawnWorkerAction(team, None, x, y)

    def game_start(self, game: Game):
        super().game_start(game)
        self._units_last = 0
        self._city_tiles_last = 0
        self._fuel_collected_last = 0
    ...
    def get_observation(self, game, unit, city_tile, team, is_new_turn):
        ...
        observation_index = 0
        if is_new_turn:
            self.update_game_state(game)  # Gets called at beginning of turn with all info from previous turn
    ...
    def get_reward(self, game, is_game_finished, is_new_turn, is_game_error):
        ...
        # rewards["rew/r_units"] = (unit_count - self._units_last) * 0.05
        rewards["rew/r_units"] = (unit_count - self._units_last) * 0.05
        self._units_last = unit_count

        # Give a reward for city creation/death. 0.1 reward per city.
        rewards["rew/r_city_tiles"] = (city_tile_count - self._city_tiles_last) * 0.1
        self._city_tiles_last = city_tile_count

        # Reward collecting fuel
        fuel_collected = game.stats["teamStats"][self.team]["fuelGenerated"]
        rewards["rew/r_fuel_collected"] = ((fuel_collected - self._fuel_collected_last) / 20000)
        self._fuel_collected_last = fuel_collected
